...
namespace WinformSqliteConf
{
    public class SQLiteDatabase
	{
		...
		public string DBLocation { get; protected set; }
		...
		public int LastInsertID
		{
			get
			{
                ...
                */
                ...
                try
                {
                    n = int.Parse(dt.Rows[0]["rv"].ToString());
                }
                catch { }
		...
		public bool Create(string filename, bool overwriteFile, string password, bool useAppSettings)
		{
        ...
        {
            return DoQuery(prepareQuery(query, args));
        }
        ...
        {
            return this.DoQueryCount(prepareQuery(query, args));
        }

		public bool DoQueryExist(string query) 
        {
            return (this.DoQueryCount(query) > 0);
        }

		public bool DoQueryExist(string query, params string[] args) 
        { 
            return this.DoQueryExist(prepareQuery(query, args));
        }
		...
		public string DoQuerySingle(string query)
		{
			...
			if (dt.Rows[0][0] is byte[])
			{
                ...
                return Encoding.UTF8.GetString(dt.Rows[0][0] as byte[],0,0);
			}
			else
			{
        ...
        {
            return this.DoQuerySingle(prepareQuery(query, args));
        }
        ...
        {
            return this.DoNonQuery(prepareQuery(query, args));
        }
		...
		public bool SetConfig(string name, bool value) 
        { 
            return this.SetConfig(name, (value ? "1" : "0"));
        }

		public bool SetConfig(string name, int value) 
        {
            return this.SetConfig(name, value.ToString());
        }
		...
		{
            string rv = this.GetConfig(name, defaultValue.ToString());
		...
		public bool GetBoolConfig(string name, bool defaultValue)
		{
			string rv = this.GetConfig(name,defaultValue.ToString());
			if (string.IsNullOrEmpty(rv))
			{
		...
		protected string escapeSQL(string q) 
        {
		...
		protected string encode64(string text) 
        {
